...
(() => {
  ...
  window.openPlatformHelp=function(){modalBody.innerHTML=`<h2>How to play PhyloGenome online</h2>
    ...
    <h3>Timeline</h3>
    <p>Choose a category to order the cards; it stays active for the game. On your turn, draw a card and see only its upper half. Place it using a plus sign in the horizontal timeline, then confirm whether its position is correct. A correct card stays in the timeline and you play again; an incorrect card returns to the bottom of your deck and the turn passes on. You can click any timeline card to read it. Changing category resets the timeline. The player with the fewest cards left wins.</p>
    <p><b>Note:</b> the platform works best while players keep an audio call open, especially in the original PhyloGenome mode.</p>`;modal.showModal();};
